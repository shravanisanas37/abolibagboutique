...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let products = [];
    const storedProducts = localStorage.getItem('aboli_bag_products');
    if (storedProducts) {
        try {
            // Force clear storage if it contains old wine bottle data references
            if (storedProducts.includes("photo-1590874103328-eac38a683ce7") || storedProducts.includes("1590874103328")) {
                localStorage.removeItem('aboli_bag_products');
                products = [...defaultProducts];
                localStorage.setItem('aboli_bag_products', JSON.stringify(products));
            } else {
    ...
    // ==========================================================================
    // 15. Admin System Logic
    // ==========================================================================
    
    // --- Firebase Configuration ---
    // NOTE: Replace these values with your actual configuration keys from the Firebase Console.
    ...
    // Initialize Firebase if the SDK is imported
    let auth = null;
    if (typeof firebase !== 'undefined') {
        firebase.initializeApp(firebaseConfig);
        auth = firebase.auth();
    } else {
        console.warn("Firebase SDK was not loaded. Admin login will fallback to dummy client check.");
    }
    ...
    // Product editor form elements
    const productEditorForm = document.getElementById('product-editor-form');
    const editorProductId = document.getElementById('editor-product-id');
    const editorFormTitle = document.getElementById('product-form-title');
    const editorName = document.getElementById('editor-name');
    const editorCategory = document.getElementById('editor-category');
    const editorSubcategory = document.getElementById('editor-subcategory');
    const editorTag = document.getElementById('editor-tag');
    const editorPrice = document.getElementById('editor-price');
    const editorOriginalPrice = document.getElementById('editor-original-price');
    ...
    const editorDimensions = document.getElementById('editor-dimensions');
    const editorColors = document.getElementById('editor-colors');
    const editorImage = document.getElementById('editor-image');
    const editorThumbnails = document.getElementById('editor-thumbnails');
    const editorImageUpload = document.getElementById('editor-file-upload');
    const imageUploadPreviewWrap = document.getElementById('image-upload-preview-wrap');
    const imageUploadPreview = document.getElementById('image-upload-preview');

    // Define subcategories for dropdown sync
    const subcategoryMap = {
        bags: ['Tote', 'Clutch', 'Sling', 'Potli', 'Wallet'],
        jewellery: ['Necklaces', 'Earrings', 'Bangles', 'Rings']
    };

    // Listen for file upload and convert to Base64 data URL
    if (editorImageUpload) {
        editorImageUpload.addEventListener('change', (e) => {
            ...
            if (file) {
                ...
                reader.onload = (event) => {
                    editorImage.value = event.target.result; // Set Base64
                    imageUploadPreview.src = event.target.result;
                    imageUploadPreviewWrap.style.display = 'flex';
                };
    ...
    // Global helper to remove uploaded image
    window.removeUploadedImage = (e) => {
        if (e) e.preventDefault();
        editorImageUpload.value = "";
        editorImage.value = "";
        imageUploadPreview.src = "";
        imageUploadPreviewWrap.style.display = 'none';
    };

    // Open/Close Admin Login Modal
    window.openAdminLoginModal = (e) => {
        if (e) e.preventDefault();
    ...
    window.closeAdminLoginModalOutside = (e) => {
    ...
    if (adminLoginForm) {
        adminLoginForm.addEventListener('submit', (e) => {
            ...
            // Clear previous errors/statuses
            loginErrorMessage.style.display = 'none';
            loginStatusMessage.style.display = 'none';

            // Check if Firebase is successfully configured
            const isFirebaseConfigured = firebaseConfig.apiKey && firebaseConfig.apiKey !== "YOUR_API_KEY";

            if (auth && isFirebaseConfigured) {
                // Production-grade secure Firebase Login
                auth.signInWithEmailAndPassword(email, password)
                    ...
                    .catch((error) => {
                        ...
                        loginErrorMessage.innerHTML = `<i class="fas fa-exclamation-circle"></i> ${message}`;
                        loginErrorMessage.style.display = 'block';
                    });
            } else {
                // Fallback to local check if Firebase is not configured yet
                if (email === 'abolibagboutique@gmail.com' && password === 'prajakta@2026') {
                    closeAdminLoginModal();
                    openAdminDashboard();
                } else {
                    loginErrorMessage.innerHTML = `<i class="fas fa-exclamation-circle"></i> Invalid email or password.`;
                    loginErrorMessage.style.display = 'block';
                }
            }
        });
    ...
    window.triggerForgotPassword = (e) => {
        if (e) e.preventDefault();
        ...
        if (!email) {
            loginErrorMessage.innerHTML = `<i class="fas fa-exclamation-circle"></i> Please enter your email address in the input field first.`;
            loginErrorMessage.style.display = 'block';
            return;
        }
        ...
        if (auth && isFirebaseConfigured) {
            auth.sendPasswordResetEmail(email)
                .then(() => {
                    loginStatusMessage.innerHTML = `<i class="fas fa-check-circle"></i> A secure password reset link has been sent to <strong>${email}</strong>. Please check your inbox.`;
                    loginStatusMessage.style.color = "#2ecc71";
                    loginStatusMessage.style.display = 'block';
                })
                .catch((error) => {
                    console.error("Reset failed:", error);
                    loginErrorMessage.innerHTML = `<i class="fas fa-exclamation-circle"></i> ${error.message}`;
                    loginErrorMessage.style.display = 'block';
                });
        } else {
            // Local Mode mock alert
            loginStatusMessage.innerHTML = `<i class="fas fa-info-circle"></i> Local Mode: A reset link would be sent to <strong>${email}</strong>. (Enable Firebase to activate email recovery).`;
            loginStatusMessage.style.color = "var(--secondary-color)";
            loginStatusMessage.style.display = 'block';
        }
    };
    ...
    // Subcategory Dropdown synchronization in the editor
    window.updateEditorSubcategories = () => {
        const cat = editorCategory.value;
        const subcats = subcategoryMap[cat] || [];
        editorSubcategory.innerHTML = "";
        subcats.forEach(sub => {
            const opt = document.createElement('option');
            opt.value = sub;
            opt.textContent = sub;
            editorSubcategory.appendChild(opt);
        });
    };

    // Product editor modal controls
    window.openProductFormModal = (mode, prodId = null) => {
        productEditorForm.reset();
        updateEditorSubcategories(); // populate initial subcategories list

        if (mode === 'add') {
            editorFormTitle.textContent = "Add New Product";
            editorProductId.value = "";
            imageUploadPreviewWrap.style.display = 'none';
            imageUploadPreview.src = "";
            editorImageUpload.value = "";
        } else if (mode === 'edit' && prodId) {
            editorFormTitle.textContent = "Edit Product Details";
            const prod = products.find(p => p.id === parseInt(prodId));
            if (!prod) return;

            editorProductId.value = prod.id;
            editorName.value = prod.name;
            editorCategory.value = prod.category;
            
            // Re-sync subcategories and set active value
            updateEditorSubcategories();
            editorSubcategory.value = prod.subcategory;
            
            editorTag.value = prod.tag || "";
            editorPrice.value = prod.price;
            editorOriginalPrice.value = prod.originalPrice || "";
            editorDescription.value = prod.description;
            editorMaterial.value = prod.material || "";
            editorDimensions.value = prod.dimensions || "";
            editorColors.value = prod.colors || "";
            editorImage.value = prod.image;
            editorThumbnails.value = prod.thumbnails ? prod.thumbnails.join('\n') : "";

            if (prod.image && prod.image.startsWith('data:image/')) {
                imageUploadPreview.src = prod.image;
                imageUploadPreviewWrap.style.display = 'flex';
            } else {
                imageUploadPreviewWrap.style.display = 'none';
                imageUploadPreview.src = "";
    ...
    window.closeProductFormModalOutside = (e) => {
        if (e.target === productFormModal) {
            closeProductFormModal();
        }
    };

    // Render products list in the admin panel
    function renderAdminProductsTable() {
        ...
        products.forEach(prod => {
            ...
            // Badge selector text helper
            let badgeClass = "badge-none";
            let badgeText = "Regular";
            if (prod.tag === 'new') { badgeClass = "badge-new"; badgeText = "New"; }
            else if (prod.tag === 'sale') { badgeClass = "badge-sale"; badgeText = "Offer"; }
            else if (prod.tag === 'popular') { badgeClass = "badge-popular"; badgeText = "Hot"; }
            ...
            tr.innerHTML = `
                ...
                <td>
                    <p class="admin-table-title">${prod.name}</p>
                    <p class="admin-table-sub">Subcategory: <strong>${prod.subcategory}</strong></p>
                </td>
                <td style="text-transform: capitalize;">
                    ${prod.category === 'bags' ? 'Bags Boutique' : 'Designer Jewellery'}
                </td>
                <td>
                    <strong>₹${prod.price}</strong>
                    ${prod.originalPrice ? `<br><span style="text-decoration:line-through; font-size:0.8rem; color:var(--text-light);">₹${prod.originalPrice}</span>` : ''}
                </td>
                <td>
                    <span class="admin-badge ${badgeClass}">${badgeText}</span>
                </td>
                <td style="text-align: center;">
                    <button class="admin-btn-action edit" onclick="openProductFormModal('edit', ${prod.id})" title="Edit Details">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="admin-btn-action delete" onclick="deleteProductItem(${prod.id})" title="Delete Item">
                        <i class="fas fa-trash"></i>
                    </button>
                </td>
            `;
            adminProductRows.appendChild(tr);
        });
    }

    // Save item (Create or Update) handler
    if (productEditorForm) {
        productEditorForm.addEventListener('submit', (e) => {
            e.preventDefault();
            
            const idVal = editorProductId.value;
            const name = editorName.value.trim();
            const category = editorCategory.value;
            const subcategory = editorSubcategory.value;
            const tag = editorTag.value;
            const price = parseInt(editorPrice.value);
            const originalPriceVal = editorOriginalPrice.value ? parseInt(editorOriginalPrice.value) : null;
            const description = editorDescription.value.trim();
            const material = editorMaterial.value.trim();
            const dimensions = editorDimensions.value.trim();
            const colors = editorColors.value.trim();
            const image = editorImage.value.trim();
            
            // Extract thumbnails array from textarea lines
            const thumbsVal = editorThumbnails.value.trim();
            const thumbnails = thumbsVal ? thumbsVal.split('\n').map(url => url.trim()).filter(url => url !== "") : [image];
            ...
            // Re-render shop and dashboard tables
            renderProducts();
            renderAdminProductsTable();
            closeProductFormModal();

            alert(idVal ? "Product updated successfully!" : "Product added successfully!");
        });
    }
    ...
    window.deleteProductItem = (id) => {
        ...
        const confirmDelete = confirm(`Are you sure you want to delete the item "${prod.name}"?`);
        if (confirmDelete) {
    ...
    // Reset database back to hardcoded defaults
    window.resetProductsDatabase = () => {
        const confirmReset = confirm("Are you sure you want to reset the catalogue to default items? All additions and edits will be lost.");
        if (confirmReset) {
            products = [...defaultProducts];
            localStorage.setItem('aboli_bag_products', JSON.stringify(products));
